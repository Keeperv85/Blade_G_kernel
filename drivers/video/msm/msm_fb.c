...
#define ZTE_BACKLIGHT_WORKAROUND
#endif
#ifdef ZTE_BACKLIGHT_WORKAROUND
struct zte_backlight_workaround_data_type {
...
DEFINE_SEMAPHORE(zte_back_workaround_sem);
static struct zte_backlight_workaround_data_type zte_backlight_workaround_data;
static void backlight_fix_worker(struct work_struct *work);
#endif
					...
					enum led_brightness value)
{
	...
	#ifdef ZTE_BACKLIGHT_WORKAROUND
	zte_backlight_workaround_data.mfd= mfd;
	zte_backlight_workaround_data.value = bl_lvl;
	up(&zte_back_workaround_sem);
	zte_backlight_workaround_data.pending = true;
	cancel_delayed_work_sync(&(zte_backlight_workaround_data.backlight_work));
	zte_backlight_workaround_data.pending = false;
	down(&zte_back_workaround_sem);
	schedule_delayed_work(&(zte_backlight_workaround_data.backlight_work), HZ/10);	
	#endif
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if (!lcd_backlight_registered) {
		...
		#ifdef ZTE_BACKLIGHT_WORKAROUND
		INIT_DELAYED_WORK(&(zte_backlight_workaround_data.backlight_work), backlight_fix_worker);
		#endif
...
static void backlight_fix_worker(struct work_struct *work)
{
	...
	struct zte_backlight_workaround_data_type *bdata = container_of(dwork,
				struct zte_backlight_workaround_data_type, backlight_work);
	
	if(!(bdata->mfd->panel_power_on) || !bl_updated ||zte_earlysuspend_suspended()) {
		up(&zte_back_workaround_sem);
		if(zte_backlight_workaround_data.pending == false)
			schedule_delayed_work(&(zte_backlight_workaround_data.backlight_work), HZ/10);
		down(&zte_back_workaround_sem);
		return;
	}
	...
	if(bdata->value == 32)
		msm_fb_set_backlight(bdata->mfd, bdata->value);
}
#endif
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		#ifndef ZTE_BACKLIGHT_WORKAROUND
		...
		#else
		if ((bl_level_old == temp) && (bl_level_old!=32)) {
			up(&mfd->sem);
			//printk( " PM_DEBUG_MXP:bl_level_old == temp,return.\n");
			return;
		}
			    ...
			    boolean op_enable)
{
	...
	//printk( " PM_DEBUG_MXP:blank_mode = %d.\n",blank_mode);
	switch (blank_mode) {
        ...
        char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	switch(LcdPanleID)
	{
		...
		case LCD_PANNEL_4P5_HX8379_IPS:
			strcpy(module_name,"IC:HX8379+TM; Glass:TFT; Resolution:480*854");
			break;	
//wangminrong for f03 qhd lcd 
		...
		case LCD_PANEL_4P5_NT35516_TIANMA_QHD:
			...
			break;	
		case LCD_PANEL_7P0_NT51008_TIANMA_WSVGA:
			strcpy(module_name,"IC:NT35516+NT52002; Glass:TFT;Resolution:1024*600"); 
			break;
		...
		case LCD_PANEL_4P5_OTM8018B_BOE://lijiangshuo added for otm8018b_boe_4p5 20130705
			...
			break;
		case LCD_PANEL_4P0_ILI9806C_TKC:
			strcpy(module_name, "IC:ILI9806C+TKC; Glass:TFT; Resolution:480*800");// lijiangshuo add for ili9806c_tkc 20130710
			break;
		case LCD_PANEL_4P0_ILI9806C_AZET:
			strcpy(module_name, "IC:ILI9806C+AZET; Glass:TFT; Resolution:480*800");// lijiangshuo add for ili9806c_tkc 20130710
			break;
		case LCD_PANEL_MAX:
